...
export default function MenuItemCosting({ item, onClose }: Props) {
  ...
  const toggleComplete = async () => {
    ...
    try {
      ...
      await logActivity(
        next ? 'Recipe Costing Marked Complete' : 'Recipe Costing Reopened',
        item.name,
        'menu'
      );
  ...
  const addLine = async () => {
    ...
    try {
      ...
      await logActivity('Recipe Ingredient Added', `${item.name} ← ${ing.ingredient_name} (${newLine.portion_g}${ing.unit})`, 'menu');
  ...
  const removeLine = async (idx: number) => {
    ...
    try {
      await setDoc(recipeDocRef, { menu_item_id: docId, lines: updated }, { merge: true });
      await logActivity('Recipe Ingredient Removed', `${item.name} — removed ${removed?.ingredient_name}`, 'menu');
        ...
        <div className="flex-1 px-6 py-5 space-y-6">
          ...
          <div className="bg-gray-50 rounded-2xl p-5">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-0.5">฿{menuPrice.toFixed(0)}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold">Food Cost</p>
                {cost !== null
                  ? <p className="text-2xl font-bold text-red-500 mt-0.5">฿{cost.toFixed(2)}</p>
            ...
            {cost !== null && menuPrice > 0 && (
              <div className="mt-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <FoodCostBadge cost={cost} price={menuPrice} />
                </div>
                <p className="text-sm text-gray-500">
                  Profit per dish: <span className="font-bold text-gray-900">฿{(menuPrice - cost).toFixed(2)}</span>
                </p>
              </div>
            )}
            {unknownCost && (
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
                  costingComplete
                    ? 'bg-[#1DA0A8] text-white hover:bg-[#18919a]'
                    : 'bg-white border-2 border-gray-200 text-gray-400 hover:border-[#1DA0A8] hover:text-[#1DA0A8]'
                }`}
              >
                {costingComplete
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between bg-white border border-gray-100 rounded-xl px-4 py-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-sm text-gray-900 truncate">{line.ingredient_name}</p>
                      ...
                      <div className="flex items-center gap-3 ml-3 shrink-0">
                        {c !== null
                          ? <span className="text-sm font-bold text-red-500">฿{c.toFixed(2)}</span>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 space-y-3">
            ...
            <div className="relative">
              <input
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1DA0A8] bg-white"
              />
              {ingOpen && (
                <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-56 overflow-auto">
                  {starredIngredients
                    .filter(i => !ingQuery || (i.ingredient_name || '').toLowerCase().includes(ingQuery.toLowerCase()))
                    .map(i => (
                      <button
                        key={i.id}
                        type="button"
                        onMouseDown={() => { setNewLine(p => ({ ...p, purchase_id: i.id })); setIngOpen(false); setIngQuery(''); }}
                        className="w-full text-left px-4 py-2.5 text-sm hover:bg-[#1DA0A8]/5 border-b border-gray-50 last:border-0"
                      >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-[#1DA0A8] text-white rounded-xl text-sm font-bold hover:bg-[#18919a] disabled:opacity-40 transition-all flex items-center gap-2"
              >
                <Plus size={15} /> Add
              </button>
            </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 space-y-2">
            <div className="flex items-center justify-between">
              ...
              {notesSaved && <span className="text-xs text-[#1DA0A8] font-semibold">Saved ✓</span>}
            </div>
            <textarea
              value={notes}
              ...
              rows={4}
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#1DA0A8] bg-white resize-none leading-relaxed text-gray-700 placeholder:text-gray-300"
            />
          </div>
